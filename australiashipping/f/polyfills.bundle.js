...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_cancellablepromise__ = __webpack_require__(106);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_cancellablepromise___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_3_cancellablepromise__);
/**
...
/* WEBPACK VAR INJECTION */(function(global) {﻿global.CancellablePromise = global.CancellablePromise || (function () {
    /**
     * resolve and rejct are identical to Promise
     * task is the the reference of this Task to allow appending children Tasks.
     * @param func
     */
    function CancellablePromise(func) {
        var _this = this;
        this.cancel = function () {
            _this._status = 'cancelled';
            _this.children.forEach(function (child) { return child.cancel(); });
        };
        this.children = [];
        this.resolve = function (value) {
            //break the chain if cancelled
            if (_this._status == 'cancelled')
                return;
            _this.result = value;
            if (_this.result instanceof CancellablePromise) {
                _this._status = 'resolving';
                //console.log('continue to wait for result: ', this.result);
                _this.append(_this.result);
                _this.result.then(_this.resolve, _this.reject);
            }
            else if (_this.result instanceof Promise || (_this.result && _this.result['then'] && _this.result['catch'] && /^\s*\[object\s+Promise\]/.test(_this.result.toString()))) {
                //run promise first and call then later
                _this._status = 'resolving';
                //console.log('**** resolving promise: ', this.result.toString());
                (_this.result).then(_this.resolve, _this.reject);
            }
            else {
                _this._status = 'resolved';
                //console.log('resolved: ', this.result ? this.result.toString(): '< Nothing >');
                _this.next();
            }
        };
        this.reject = function (reason) {
            ...
            //    //console.log('continue to wait for result: ');
            //    this.reason.then(this.reject, this.reject);
            //}else
            if (_this.reason instanceof CancellablePromise) {
                _this._status = 'rejecting';
                //console.log('continue to wait for result: ');
                _this.append(_this.reason);
                _this.reason.then(_this.reject, _this.reject);
            }
            else if (_this.reason instanceof Promise || (_this.reason && _this.reason['then'] && _this.reason['catch'] && /^\s*\[object\s+Promise\]/.test(_this.reason.toString()))) {
                //run promise first and call then later
                _this._status = 'rejecting';
                //console.log('continue to wait for result: ');
                _this.reason.then(_this.reject, _this.reject);
            ...
            switch (_this._status) {
                case 'resolved':
                    if (_this.onfulfilled)
                        try {
                            var Next = _this.onfulfilled(_this.result);
                            _this.nextTask.resolve(Next); //the default behavior allows passing Promise/Task into the next one;
                        }
                        catch (e) {
                            _this.nextTask.reject(e);
                        }
                    break;
                case 'rejected':
                    if (_this.onrejected)
                        try {
                            var Next = _this.onrejected(_this.reason);
                            _this.nextTask.resolve(Next); //the default behavior allows passing Promise/Task into the next one;
                        }
                        catch (e) {
                            _this.nextTask.reject(e);
        ...
        if (func)
            func(this.resolve, this.reject, this);
    }
    CancellablePromise.prototype.append = function (task) {
        this.children.push(task);
        task.parent = this;
        return task;
    };
    CancellablePromise.prototype.clear = function () {
        this.children = [];
    };
    CancellablePromise.prototype.then = function () {
        ...
        this.onfulfilled = args[0];
        this.onrejected = args[1];
        if (this.onfulfilled || this.onrejected) {
            this.nextTask = new CancellablePromise(function (resolve, reject, task) { });
            this.append(this.nextTask);
        }
        this.next();
    ...
    Object.defineProperty(CancellablePromise.prototype, "status", {
        get: function () {
            return this._status;
        },
        enumerable: true,
    ...
    Object.defineProperty(CancellablePromise.prototype, "cancelled", {
        get: function () {
    ...
    Object.defineProperty(CancellablePromise.prototype, '[Symbol.toStringTag]', {
        get: function () {
            return 'Task';
        },
        enumerable: true,
    ...
    /**
     * append all tasks as children of the current task, and run them as promise; they can be cancelled;
     * @param tasks
     */
    CancellablePromise.prototype.all = function () {
        var _this = this;
        var tasks = [];
        for (var _i = 0; _i < arguments.length; _i++) {
            tasks[_i] = arguments[_i];
        }
        tasks.forEach(function (task) { return _this.append(task); });
